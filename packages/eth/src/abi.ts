...
export async function downloadABI(chain: ChainId,
    ...
    if (!abiRes) {
        throw Error(`Failed to download ABI for ${address} on ${chain}`)
    }
    name = abiRes.name || address

    const abiFilePath = getABIFilePath(chain, name, '', folder);
    writeABIFile(abiRes.abi, abiFilePath)
    return {file: abiFilePath, abi: abiRes, name}
}
...
export function getABIFilePath(chain: string, name: string, address?: string, folder: string = "contracts"): string {
    let subpath
    ...
    return path.join(folder, subpath, filename, 'abi.json')
}
...
export function writeABIFile(obj: string | object, output: string) {
    ...
    console.error(chalk.green('ABI has been downloaded to', output))
}
